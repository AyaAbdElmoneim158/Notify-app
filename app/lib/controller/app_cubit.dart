...
class AppCubit extends Cubit<AppStates> {
  ...
  void getTasks() async {
    ...
    unCompletedTasks.addAll(
      tasks.where((task) => task.status == 'uncompleted').toList(),
    );
    doneTasks.addAll(
      tasks.where((task) => task.status == 'completed').toList(),
    );
    favoriteTasks.addAll(
      tasks.where((task) => task.favorite == 'true').toList(),
    );
    dailyTasks.addAll(
      tasks.where((task) => task.date == date).toList(),
    );
